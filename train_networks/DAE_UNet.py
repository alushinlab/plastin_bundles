...
noise_folder = folder + 'tplastin_noise/'#'noise_proj4/'
noNoise_folder = folder + 'tplastin_noNoise/'#'noNoise_proj4/'

train, target = import_synth_data(noise_folder, noNoise_folder, 192, 0, 300000)
...
def train_model(train_data, train_target):
	...
	checkpoint = keras.callbacks.ModelCheckpoint('300ktraining_tplastin_CCC00000_best_model_so_far_fig.h5', monitor='loss', verbose=0, save_best_only=True, mode='auto',save_weights_only=False, period=True)
	history = autoencoder.fit(x=train_data, y=train_target, epochs=30, batch_size=16, verbose=1, validation_data = (val_train[1:], val_target[1:]), callbacks=[es,checkpoint])
...
model_save_name = './fig_300000training_tplastin_CCC00000.h5'
print('Model finished training.\nSaving model as ' + model_save_name)
autoencoder_three.save(model_save_name)

import pickle
with open('./DAE_trainHistoryDict_fig', 'wb') as file_pi:
