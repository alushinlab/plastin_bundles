...
def compute_bridge_length(fil1, fil2):
	fil1_482G_coord = fil1.select('chain M').getCoords()[463]
...
def measure_angles_and_dist(fil1_name, fil2_name):
	...
	fil1_139V_coord = fil1.select('chain M').getCoords()[343]-[0.0,0.0,0.7837] 
	
	# Define vectors orthogonal to the reference filament's central axis and each other
	normal_vect = fil1_COM - fil1_139V_coord
	new_normal = np.cross(fil1_central_axis, normal_vect)
...
fil1_file_names = sorted(glob.glob('../antiparallel/chunk_%s/*fil1.pdb'%(chunk_num)))
fil2_file_names = sorted(glob.glob('../antiparallel/chunk_%s/*fil2.pdb'%(chunk_num)))
results = []
